...
/*   Updated: 2024/06/08 22:19:43 by alafdili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int init_info_struct(t_pinfo **init, char **args, int ac, int *rvalue)
{
	...
	if (((*init)->die_time = ft_atoi(args[2])) == -1 || (*init)->die_time < 60)
		return (put_error("Time must be > 60"), *rvalue = 1, 1);
	...
	if (ac == 6)
	{
		if (((*init)->eat_time_nb = ft_atoi(args[5])) == -1)
			return (put_error("Time must be > 60"), *rvalue = 1, 1);
		if ((*init)->eat_time_nb == 0)
			return (*rvalue = 0, 1);
	}
	else
		(*init)->eat_time_nb = -1;
...
int pars_args(t_pinfo **init, char **args, int ac, int *rvalue)
{
	...
	while (args[i])
	{
		j = 0;
		if (args[i][0] == '\0')
			return (put_error("Invalid argument!"), *rvalue = 1, 1);
		while (args[i][j])
		{
